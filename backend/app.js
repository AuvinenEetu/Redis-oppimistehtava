...
app.post('/init-sample', async (req, res) => {
  try {
    ...
    if (uCount || eCount || bCount) {
      return res.status(400).json({ error: 'Collections not empty; init aborted', counts: { users: uCount, events: eCount, bets: bCount } });
    }
    ...
    const users = [
      { _id: user1Id, nimi: { etunimi: 'Matti', sukunimi: 'Meikäläinen' }, kayttajatunnus: 'matti_meika', sahkoposti: 'matti@gmail.fi', salasana: 'matti123' },
      { _id: user2Id, nimi: 'Maija Meikäläinen', kayttajatunnus: 'maija_meika', sahkoposti: 'maija@gmail.fi', salasana: 'maija123' },
      { _id: user3Id, nimi: 'John Doe', kayttajatunnus: 'john_doe', sahkoposti: 'john@example.com', salasana: 'john123' }
    ];
    ...
    res.json({ ok: true, inserted: { users: users.length, events: events.length, bets: bets.length } });
  } catch (err) {
...
(async () => {
  ...
  const redisDb = parseInt(process.env.REDIS_DB || process.env.REDIS_DATABASE || '0', 10);
  ...
  if (normalizedRedisUrl.startsWith('http://')) normalizedRedisUrl = normalizedRedisUrl.replace(/^http:/, 'redis:');
  if (normalizedRedisUrl.startsWith('https://')) normalizedRedisUrl = normalizedRedisUrl.replace(/^https:/, 'rediss:');
  // use normalized URL
  ...
  try {
    ...
    if(typeof redisClient.select === 'function'){
      try{ await redisClient.select(redisDb); }catch(e){ console.warn('Redis select failed', e.message) }
    }
...
async function cacheSet(key, value, ttlSec=60){
  ...
  try{ await redisClient.set(key, JSON.stringify(value), { EX: ttlSec }); }catch(e){ console.error('Redis set error', e) }
}
...
app.get('/bets', async (req, res) => {
  try {
    ...
    const bets = await Bet.find().limit(200).sort({ vedon_pvm: -1 });
    const t1 = Date.now();
...
app.get('/events', async (req, res) => {
  try {
    ...
    const cached = await cacheGet(cacheKey);
    if(cached){ res.set('X-Query-Time-ms','0'); res.set('X-Data-Source','redis'); return res.json(cached); }
    const events = await Event.find().limit(200).sort({ paiva: 1 });
    const t1 = Date.now();
    res.set('X-Query-Time-ms', String(t1 - t0));
    res.set('X-Data-Source','mongo');
...
app.get('/users', async (req, res) => {
  try {
    ...
    const cached = await cacheGet(cacheKey);
    if(cached){ res.set('X-Query-Time-ms','0'); res.set('X-Data-Source','redis'); return res.json(cached); }
    const users = await User.find().limit(200).sort({ nimi: 1 });
    const t1 = Date.now();
    res.set('X-Query-Time-ms', String(t1 - t0));
    res.set('X-Data-Source','mongo');
...
app.get('/top-events', async (req, res) => {
  try {
    ...
    const cached = await cacheGet(cacheKey);
    if(cached){ res.set('X-Query-Time-ms','0'); res.set('X-Data-Source','redis'); return res.json({ mode, results: cached }); }

    const t0 = Date.now();
    const pipeline = [
      { $group: { _id: { tapahtuma_id: '$tapahtuma_id', tapahtuman_nimi: '$tapahtuman_nimi' }, count: { $sum: 1 }, totalPanos: { $sum: { $ifNull: ['$panos', 0] } } } },
      { $sort: mode === 'stake' ? { totalPanos: -1 } : { count: -1 } },
      { $limit: 5 },
      { $project: { tapahtuma_id: '$_id.tapahtuma_id', tapahtuman_nimi: '$_id.tapahtuman_nimi', count: 1, totalPanos: 1, _id: 0 } },
      { $lookup: { from: 'events', localField: 'tapahtuma_id', foreignField: '_id', as: 'event' } },
      { $unwind: { path: '$event', preserveNullAndEmptyArrays: true } },
      { $project: { tapahtuma_id: 1, tapahtuman_nimi: 1, count: 1, totalPanos: 1, event: 1 } }
    ];
...
const mongoUri = process.env.MONGO_URI || 'mongodb://root:password@localhost:27017/vedonlyontikanta?authSource=admin';
